...
def run_vcl(hidden_size, no_epochs, data_gen, coreset_method, coreset_size=0, batch_size=None, single_head=True ,num_gauss= 1, num_train = 10, tau = 1.0,sd = 1):
	...
	for task_id in range(data_gen.max_iter):
		...
		print("seed ",sd)
		lr = 0.001
		mf_model = MFVI_NN(in_dim, hidden_size, out_dim, x_train.shape[0],no_train_samples=num_train ,prev_means=mf_weights, prev_log_variances=mf_variances,prev_coffs = mf_coffs,pre_variance_mlp = mf_variance_mlp,gauss_mixture = num_gauss, tau = tau , learning_rate= lr)
...
def create_path_file_result(num_gauss , num_train , tau , sd):
	root = "result_mixture_split_mnist_version_10task"
