...
class MFVI_NN(Cla_NN):
	...
	#     no_task = len(weight_head[0][0])
	#     print("no task ", no_task)
	#     list_weight_trainable_share = []
	#     for i in range(self.gauss_mixture):
	#         m,v,c = weight_share[i]
	#         if i == 0 :
	#             list_weight_trainable_share.append([m,v])
	#         else:
	#             list_weight_trainable_share.append([m,v,c])
	#     train_step_head = tf.train.AdamOptimizer(learning_rate).minimize(self.cost , var_list = weight_head )
	#     if no_task == 1:
	#         lr_share = 1e-3
	#     else:
	#         lr_share = self.learing_rate_share
	#     train_step_share = tf.train.AdamOptimizer(lr_share / no_task).minimize(self.cost , var_list = list_weight_trainable_share)
	#     self.train_step = [train_step_share , train_step_head]
