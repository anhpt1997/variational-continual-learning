...
class Cla_NN(object):
    ...
    def train(self, x_train, y_train, task_idx, no_epochs=1000, batch_size=100, display_epoch=30, lr = 0.001):
        ...
        for epoch in range(no_epochs):
            ...
            for i in range(total_batch):
                ...
                _,  c = sess.run(
                    ...
                    feed_dict={self.x: batch_x, self.y: batch_y, self.task_idx: task_idx, self.learning_rate : lr})
    ...
    def train_vs_validation(self, x_train, y_train,x_valid, y_valid, task_idx, no_epochs=1000, batch_size=100, display_epoch=3, lr = 0.00005):
        N = x_train.shape[0]
        if batch_size > N:
            batch_size = N
        ...
        # save best model
        best_acc = - math.inf
        num_thresol_stop_loop = 100
        lr_total = lr
        for epoch in range(no_epochs):
            if num_thresol_stop_loop < 0 :
                break
            if lr_total < 1e-10 :
                break
            # if epoch % display_epoch ==0:
            #     print("lr total ", lr_total)
            #     acc = utils_mixture.get_scores(self, x_valid, y_valid, [], [], self.hidden_size, no_epochs, True, 5000)
            #     print(epoch ," acc validation", acc , "sum acc" , np.sum(acc) )
            perm_inds = range(x_train.shape[0])
            perm_inds = np.random.permutation(perm_inds)
            cur_x_train = x_train[perm_inds]
            cur_y_train = y_train[perm_inds]
            ...
            for i in range(total_batch):
                ...
                _,  c = sess.run(
                    [self.train_step, self.cost],
                    feed_dict={self.x: batch_x, self.y: batch_y, self.task_idx: task_idx , self.learning_rate : lr_total })
                # Compute average loss
                avg_cost += c / total_batch
            #Display logs per epoch step
            if epoch % display_epoch == 0:
    ...
    def compute_valid_loss(self ,x_valid , y_valid , task_idx):
        s = self.sess
        total_valid_loss = []
        for i in range(len(x_valid)):
            total_valid_loss.append(s.run(self.cost ,feed_dict={self.x: x_valid[i], self.y: y_valid[i], self.task_idx: task_idx} ))
        return total_valid_loss

    def compute_valid_log_pred(self ,x_valid , y_valid , task_idx):
        s = self.sess
        total_valid_loss = []
        for i in range(len(x_valid)):
            total_valid_loss.append(s.run(self._logpred(x_valid[i], y_valid[i], task_idx)))
        return total_valid_loss
...
class MFVI_NN(Cla_NN):
    ...
    def __init__(self, input_size, hidden_size, output_size, training_size,
        no_train_samples=10, no_pred_samples=50, prev_means=None, prev_log_variances=None,prev_coffs= None, prior_mean=0., prior_var=1.0, gauss_mixture = 1,tau = 1.0 , task = 0):
        super(MFVI_NN, self).__init__(input_size, hidden_size, output_size, training_size)
        self.task= task
        print("self.task id ", task)
    ...
    def compute_lr(self , prev_log_variances):
        list_sum_variance = []
        for mixture in range(self.gauss_mixture):
            sum_variance = []
            for i in range(len(prev_log_variances[mixture])):
                for j in range(len(prev_log_variances[mixture][i])):
                    sum_variance .append( np.mean(prev_log_variances[mixture][i][j]))
            list_sum_variance.append(sum_variance)
        list_sum_variance = np.array(list_sum_variance)
        sum_variance = np.mean(list_sum_variance , axis = 1)
        return sum_variance
