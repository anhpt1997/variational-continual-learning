...
from cla_gauss_mixture_split_2 import MFVI_NN
...
def get_scores(model, x_testsets, y_testsets, x_coresets, y_coresets, hidden_size, no_epochs, single_head, batch_size=None):
	...
	for i in range(len(x_testsets)):
		...
		for i in range(total_batch):
			...
			pred_y = np.argmax(pred_overall, axis=1)
			y = np.argmax(y_test[start_ind : end_ind, :], axis=1)
			cur_acc += len(np.where((pred_y - y) == 0)[0]) * 1.0
			print(cur_acc , "total ", batch_x.shape[0])
